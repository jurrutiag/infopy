...
def EDGE(
    ...
    gamma=[1, 1],
    epsilon=[0, 0],
    epsilon_vector="range",
    eps_range_factor=0.1,
    normalize_epsilon=True,
    ensemble_estimation="median",
    L_ensemble=10,
    hashing="p-stable",
    stochastic=False,
):
    ...
    if epsilon[0] == 0:
        ...
        cx, cy = 18 * d_X_shrink / np.max(
            [(1 + 1.0 * math.log(dim_X)), 1]
        ), 18 * d_Y_shrink / np.max([(1 + 1.0 * math.log(dim_Y)), 1])
        ...
        cx, cy = 15 * d_X_shrink / np.max(
            [(1 + 1.0 * math.log(dim_X)), 1]
        ), 15 * d_Y_shrink / np.max([(1 + 1.0 * math.log(dim_Y)), 1])
        eps_X0, eps_Y0 = eps_X_temp * cx * gamma[0], eps_Y_temp * cy * gamma[1]
        # print('eps_X0, eps_Y0', eps_X0, eps_Y0)
